...
public class UserDao extends DAO implements IDAO<User> {
    ...
    public User login(String email, String password) throws SQLException {
        ...
        String sql =
                "SELECT u.id, u.email, u.password, u.name, u.phone, u.avatar, u.gender, u.birthday, u.active, " +
                        "       r.code AS role_code " +
                        "FROM users u " +
    ...
    public boolean updateProfile(int userId, String name, String phone, String gender, java.sql.Date birthday) {
        ...
        try (Connection cn = getConnection();
            ...
            if (birthday != null) ps.setDate(4, birthday);
    ...
    public boolean setUserRole(int userId, String roleCode) throws SQLException {
        ...
        try (Connection cn = getConnection()) {
            ...
            try (
                    ...
                    PreparedStatement ins = cn.prepareStatement("INSERT INTO user_roles(user_id, role_id) VALUES(?, ?)")
            ) {
                del.setInt(1, userId);
                del.executeUpdate();
